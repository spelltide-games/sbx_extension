...
static Variant mpack_to_gd(mpack_node_t node) {
	...
	switch (type) {
		...
		default:
			return Variant(); // should never happen
	}
}
...
static bool gd_to_mpack(Variant object, mpack_writer_t *writer) {
	switch (object.get_type()) {
		case Variant::NIL:
			...
			break;
		case Variant::BOOL:
			mpack_write_bool(writer, (bool)object);
			break;
		case Variant::INT:
			mpack_write_int(writer, (int64_t)object);
			break;
		case Variant::FLOAT:
			mpack_write_double(writer, (double)object);
			break;
		case Variant::STRING: {
...
Variant MessagePack::loads_c(const char *data, size_t size) {
	mpack_tree_t tree;
...
Variant MessagePack::loads(const PackedByteArray &arr) {
	const char *data = (const char *)arr.ptr();
	size_t size = arr.size();
	return loads_c(data, size);
}
